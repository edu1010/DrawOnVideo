...
export function buildProjectPayload({ videoPath, videoMeta, layers, videoClips, videoLayers }) {
  const serializableLayers = (layers || []).map((layer) => ({
    ...
    strokes: (layer.strokes || []).map((stroke) => ({
      ...
      pressureSensitivity: Number(stroke.pressureSensitivity) || 1.7,
      pressureCurve: Number(stroke.pressureCurve) || 1.75,
      pressureMinScale: Number(stroke.pressureMinScale) || 0.05,
  ...
  return {
    ...
    videoClips: (videoClips || []).map((clip) => ({
      ...
      timelineDurationMs: Number(clip.timelineDurationMs) || 0,
      ...
      fadeInDurationMs: Number(clip.fadeInDurationMs) || 0,
      fadeOutDurationMs: Number(clip.fadeOutDurationMs) || 0,
      audioFadeInDurationMs: Number(clip.audioFadeInDurationMs) || 0,
      audioFadeOutDurationMs: Number(clip.audioFadeOutDurationMs) || 0,
